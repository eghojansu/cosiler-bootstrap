...
guard('admin');
not_found_if(!($user = get_one('user', array('userid <> ? and userid = ?', user_id(), $params['user']))));
menu_active('user');

$data = data() ?? $user;
$error = error();
?>
<h4 class="border-bottom p-3">Edit User</h4>
...
<form method="post" style="max-width: 500px">
  ...
  <div class="row mt-3">
    ...
    <div class="col-sm-9">
      <?= choice('roles', $fun['choice.roles'], explode(',', $data['roles'] ?? ''), null, array(isset($error['errors']['roles']) ? 'is-invalid' : null), true, true) ?>
